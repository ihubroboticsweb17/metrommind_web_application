...
const HeroSection = ({ theme = "default" }: HeroSectionProps) => {
  ...
  // Check session status and report generation
  useEffect(() => {
    const reportGenerated = localStorage.getItem('ai_report_generated');
  ...
  const getPrimaryButtonConfig = () => {
    if (!isLoggedIn) {
      return {
        text: "Sign in",
        path: "/login",
        icon: <ArrowRight className="inline-block ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
      };
    }
      ...
      <div className="metro-container">
        <div className="text-center ">
          ...
          {isLoggedIn && hasActiveSession && !hasGeneratedReport && (
            <div className="mt-4 inline-flex items-center px-3 py-1 rounded-full bg-orange-100 text-orange-800 text-sm">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8">
            <Link
              ...
              className={`px-6 py-2.5 rounded-md group font-medium transition-all duration-300 
                ${getButtonClass()} inline-flex items-center relative`}
            >
              {buttonConfig.text}
              {buttonConfig.icon}
