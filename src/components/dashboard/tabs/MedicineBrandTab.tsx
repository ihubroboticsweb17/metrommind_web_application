...
interface MedicineBrandTabProps {
  refreshTrigger: boolean;
}
const MedicineBrandTab: React.FC<MedicineBrandTabProps> = ({ refreshTrigger }) => {
  ...
  const fetchMedicinebrandlist = async () => {
    try {
      setLoading(true);
      const data = await MedicineBrandlist();
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      fetchMedicinebrandlist(); // Now works ✅
    } catch (err) {
      toast({
        title: "Error",
...
export default MedicineBrandTab;
